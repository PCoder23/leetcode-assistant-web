...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.size > MAX_SIZE_BYTES) {
      return NextResponse.json(
        {
          error: `Audio file too large. Max allowed size is ${MAX_SIZE_MB} MB.`,
        },
        { status: 413, headers: { "Access-Control-Allow-Origin": "*" } } // 413 = Payload Too Large
      );
    }
    ...
    const filename = `recording-${randomUUID()}.wav`;
    ...
    } = supabaseServer.storage.from("recordings").getPublicUrl(filename);
    ...
    // 🔹 Step 2: Generate Feedback with GPT-4o-mini
    ...
    - All scores must be integers from 0 to 10 (not percentages).
    ...
    **NOTES**
    ...
    {"scores": {
        "understanding": integer from 0 to 10,
        "approach": integer from 0 to 10,
        "communication": integer from 0 to 10,
        "code": integer from 0 to 10
      },
      "summary": string,
      "highlights": [string],
      "gaps": [string],
